...
async function startBot() {
  ...
  // Check if sessionID is provided and process NOVALANDXBOT! format session
  if (config.sessionID && config.sessionID.startsWith('NOVALANDXBOT!')) {
    try {
      const [header, b64data] = config.sessionID.split('!');

      if (header !== 'NOVALANDXBOT' || !b64data) {
        throw new Error("❌ Invalid session format. Expected 'NOVALANDXBOT!.....'");
      }
      ...
      console.log('📡 Session : 🔑 Retrieved from NOVALANDXBOT Session');

    } catch (e) {
      console.error('📡 Session : ❌ Error processing NOVALANDXBOT session:', e.message);
